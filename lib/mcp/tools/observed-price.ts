...
export const analyzeObservedPriceTool = defineTool(
  "analyze_observed_price",
  {
    ...
    description:
      "分析單一目前上市櫃公司的 caller-supplied 觀察價格，相對於 TWSE／TPEx 官方最近完成交易日 raw_unadjusted 收盤價的絕對與百分比差異。caller 必須明示 observed_price_twd、含 Z 或 UTC offset 的 observed_at，以及 source_label；觀察值不會被冒充成官方、即時或 real-time 行情。工具先以 market=all 目前公司 master 核對 identity，並在 request start 固定 evaluatedAt；接著由公司所屬市場的官方年度開休市日曆與 exact benchmark session 解析 authoritative expectedAsOf，再只查該日 exact single-stock OHLC。resolver、selected bar 與公開 close date 必須完全一致；unresolved、缺少 exact bar、identity 不符或無有效正數 close 時 fail closed，不會退回可能落後的全市場 latest snapshot 或前一日價格。同日比較須晚於 13:33 Asia/Taipei 的保守 regular-session completion guard。輸出分開 CALLER_SUPPLIED、OFFICIAL_MASTER_RAW、OFFICIAL_MARKET_RAW 與 MOPSFIN_CALC provenance，保留 listed／otc master、單股官方價格來源、cache refresh、dependency ledger、官方 history cutoff 與 resolver evidence 的實際／最大 logical source-load budget。價差只是機械比較，不是 fair value、合理進場區、adjusted close、目標價、買賣評級或投資建議；本工具也不是外部盤中 quote provider。",
