...
估值研究路由：需要單一非金融公司的 normalized TTM／FCFF proxy／net-debt／official-close evidence 時先用 get_valuation_model_inputs；需要在完整顯性假設下，一次反解目前官方收盤價隱含的 revenue CAGR、FCFF CAGR 或 terminal operating margin 時再用 run_reverse_dcf。後者是 market-implied condition，不是 intrinsic value、共識或目標價。
...
Caller 觀察價：analyze_observed_price 只在 caller 已提供單一 company_code、正的 observed_price_twd、明確 offset ISO observed_at 與 source_label 時使用。它以 market=all 目前公司 master 核對 identity；價格 dependency 使用 compatible current-master reconciliation，各市場 matchRatio 至少 95% 才接受，非目標公司的母體差異不阻斷查詢，但指定公司的 code、name、market 必須由外層 master 與官方行情列精確吻合。同一台北日期必須到 13:33（含）後才比較，以保守涵蓋暫緩收盤。-00:00 是未知 offset，不能當成明確時區。輸出分開 CALLER_SUPPLIED、OFFICIAL_MASTER_RAW、OFFICIAL_MARKET_RAW 與 MOPSFIN_CALC，保留三份 evidence sources、cache 時序、officialHistoryCutoff 與 dependencyLedger；nested compatible-master acquisition source evidence 未由價格 dependency 暴露時必須明示，不能以 orchestration master 冒充。這不是外部 quote provider、官方／real-time 行情、fair value、進場區、目標價、評級或投資建議。
...
Freshness 與時間血緣：latest 只表示查詢意圖，不會直接宣稱資料在預期窗口內。meta.asOf 分開提供 data cutoff、真正 upstream retrievedAt、MCP servedAt 與 caller-specific cache provenance；cache hit 不會改寫 retrievedAt，官方未明示 publishedAt 時維持 null。meta.quality.freshnessDetails 以中央 source-specific policy 列出 observedAsOf、expectedAsOf、lag 與 reason；官方 completed-session policy 使用 market-specific TWSE／TPEx 年度開休市日曆、當月 exact benchmark marker 與 13:33 Asia/Taipei guard，並揭露 resolver evidence 與 freshness_meta_layer actual／maximum logical-load budget。resolver 不可用、契約漂移、marker 未確認、跨市場日期不一致，或其他 policy 沒有可靠 expected as-of 時固定 unknown 並帶 FRESHNESS_UNVERIFIED；落後 policy 時為 stale 並帶 DATA_STALE。歷史 exact/range selector 的 latest freshness 為 not_applicable，且不呼叫 completed-session resolver。
...
export const MOPSFIN_OFFICIAL_GUIDANCE = {
  ...
  valueBasis: [
    ...
    },
    {
      ...
      basis:
        "analyze_observed_price 將 caller-supplied 價格與時間保留為未獨立驗證的 CALLER_SUPPLIED evidence，另以官方 current master 與最近 completed-session raw close 建立基準，再做機械價差；不是官方即時行情、fair value 或投資建議。",
    },
    {
      dataType: "市場隱含 Reverse DCF",
      basis:
        "run_reverse_dcf 以官方最近完成交易日收盤與 normalized FCFF facts，在 caller 明示全部 forward／EV-bridge assumptions 下，一次反解一個市場隱含變數；不是 intrinsic value、共識、目標價或投資建議。",
    },
    {
  ...
  interpretationNotes: [
    ...
    "analyze_observed_price 的 observedPriceTwd／observedAt 來自 caller，必須與 official completed close 及 calculated difference 分開引用；不得把結果描述為官方即時行情、合理價或投資建議。",
