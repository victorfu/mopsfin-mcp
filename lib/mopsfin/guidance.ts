...
使用順序：對指定公司查核指定日期範圍內的官方重大訊息與法人說明會時使用 get_company_catalyst_events；查核當期財測達成／重大差異、股東會與股利決議 snapshot 時使用 get_company_catalyst_snapshots。需要目前上市櫃母體或自行掃描候選代號時先呼叫 list_companies；只知道特定公司名稱或代號時使用 find_companies，不要用 find_companies 枚舉全市場。list_companies 的 market=listed 只回上市（含創新板）、market=otc 只回上櫃、market=all 合併兩個當次官方來源；include_financial=false 或 include_ky=false 可排除金融保險業或 KY 公司。自行控制單一股票 raw 歷史分頁時使用 get_stock_ohlc；需要一次收齊最多 36 個月的 raw 或 price-index-compatible 公司行動調整價格序列時使用 get_stock_price_series；查同一交易日 accepted market snapshot 或一批代號使用 get_daily_market_ohlc，並依 universeCoverageVerified／reconciliation 判讀 rowset；caller 已自行取得一筆觀察價、需要與官方最近完成交易日收盤分開比較時使用 analyze_observed_price，不得把 caller 值稱為官方或 real-time quote；比較個股與市場在 5／20／60／120 個交易日視窗的報酬、量能與價格路徑代理訊號使用 get_stock_reaction_signals。查 latest 或指定日估值使用 get_daily_market_valuation；需要單一非金融公司的 TTM、historical FCFF proxy、net debt、issued shares、official close、market cap 與 enterprise value 可追溯模型資料層時使用 get_valuation_model_inputs，但它不執行 DCF。查單月營收使用 get_monthly_revenue；查 3–24 個月營收序列與透明衍生值使用 get_monthly_revenue_trend。不知道 metric_code、industry_codes、institution_codes 或可用期別時先呼叫 list_catalog；單一指標使用 get_company_metric，多家公司 × 多指標使用 get_company_metrics_batch。回答前應讀取 list_catalog guidance 以及每次結果的 meta、coverage、status 與 warnings。

研究工具：先以 list_catalog(kind=research_fields) 查固定欄位、單位、filterOperators、sortable 與 cost；這個模式不查上游，kind=all 另附動態 financial.<metric_code> 欄位。screen_companies 對目前母體作最多 12 個 AND 條件、24 欄、3 個排序鍵的篩選；matched/notMatched/undetermined 分開，null、stale、unavailable 不當零，全體先排序後分頁，cursor 綁來源內容更正。compare_companies 比較 1–20 家、最多 8 個季度指標；common_latest 僅搜尋最近 12 個已完成曆季，缺值或來源失敗公司不被移除；無交集回 no_common_period，不回退 company_latest。金融淨收益與一般營收分開 definitionId，未知業別映射不混合統計。兩者 output_mode=full/compact/summary，summary scope 明示全選取集合或本頁；保留 metadata、sources 與 warnings。get_stock_technicals 計算單一公司的日線 SMA、Wilder RSI14、20/60 日樣本 log-return 波動率、突破與前 20 日量比；RSI 固定 251 closes、波動率年化使用 sqrt(252)，突破與量比分母排除今日。依官方 benchmark sessions 驗證缺日、最多 18 月暖機，adjusted 缺值不回退 raw。這些工具不提供歷史 point-in-time vintage、intraday、total return 或買賣評級。

輸出模式：get_daily_market_ohlc、get_daily_market_valuation、get_monthly_revenue 支援 columns 與 output_mode=full/compact/summary；summary 是分頁前完整選取集合，compact 或 columns 投影是本頁。get_company_metrics_batch 的 compact/summary 僅限本頁，保留 meta.page.next，不能說成全部 requested 公司統計。上述工具省略新參數或僅指定 full 時保留原契約；投影使用 presentation 並明示省略原 rows/bars/companies，null 與來源品質仍保留。get_stock_price_series 支援 full/summary：summary 省略 bars，保留 coverage、eventLedger、sources、warnings；端點報酬只描述觀察首末 close，最大回撤與每日 log-return 樣本標準差需驗證完整官方 session grid，後者不年化。摘要另查最多 36 月 benchmark，失敗／缺日／adjusted 缺值不回退、不補零；summarySources、summaryFailure、summaryWorkBudget 揭露額外證據與成本。精簡輸出只降低回傳量，不保證降低上游成本。

產業統計時間：get_industry_data 的 statistics 模式以 query.period 表示上游已核對的季度，meta.asOf 與 freshness observedAsOf 使用該季度；periods 與 series[].points[].period 是產業分類，不可當作日期。trend 模式才使用跨期季度時間軸。

估值研究路由：需要單一非金融公司的 normalized TTM／FCFF proxy／net-debt／official-close evidence 時先用 get_valuation_model_inputs；需要在完整顯性假設下，一次反解目前官方收盤價隱含的 revenue CAGR、FCFF CAGR 或 terminal operating margin 時再用 run_reverse_dcf。兩者的 latest completed close 都先由 company-market authoritative resolver 固定 expectedAsOf，再只接受 exact single-stock monthly OHLC 中 selectedBarDate 完全相等的 bar，不使用全市場 latest 日期或前一日 fallback。後者是 market-implied condition，不是 intrinsic value、共識或目標價。

官方事件：get_company_catalyst_events 查詢 1–20 家 selected companies 在明確 YYYY-MM-DD 起訖範圍內的官方重大訊息與法人說明會，含首尾最多 366 日。重大訊息使用 MOPS 歷史查詢並在近期範圍以 TWSE／TPEx current OpenAPI 補強，法說會使用 MOPS 歷史日曆；公開介面只支援 material_information 與 investor_conference。目前 master 只協助 current identity 與近期重大訊息市場路由；歷史法說固定查上市／上櫃兩市場，避免轉板漏件。catalyst 計畫查詢工作單位上限為 40：重大訊息每個 company×month 一單位、歷史法說每個 company×month×market 一單位，再加近期 market snapshot；這不包含 master hint、cache／single-flight 或 retry attempts，也不是實際 HTTP attempt 計數。失敗依 per_company_event_type_calendar_month 隔離；familyCoverage 只計歷史月份，近期補強另列 coverage.currentSnapshots。只有官方明確空回應可驗證、所有 requested families complete、companies[].eventCount=0、沒有 failures 且 meta.quality.selection=complete 已確認 identity 時，才可稱為該公司在 requested range 已驗證無事件；上游、security block 或 parser failure 不是空值。publishedAt、factDate、scheduledAt、effectiveAt 必須分開解讀；dateConfidence=confirmed 只表示官方時間證據，不表示正面、負面或市場尚未反應。本工具 isConsensus 固定 false，不提供公司財測、分析師 consensus／預估修正、情緒、impact score、目標價或投資建議。offset 續頁會重新查詢官方來源，不是 pinned point-in-time snapshot；必須沿用相同 query 並比對 fingerprint 與 meta.asOf.snapshotId，改變時從 offset=0 重查。歷史事件 fingerprint 包含全部正規化事件內容，地點、連結、備註或標題更正均可偵測；取得時間及 offset 不影響內容版本。
...
Caller 觀察價：analyze_observed_price 只在 caller 已提供單一 company_code、正的 observed_price_twd、明確 offset ISO observed_at 與 source_label 時使用。它以 market=all 目前公司 master 核對指定公司的 code、short name、market 與 exchange；listed／otc 來源各自驗證 schema、coverage、reportDate 與 provenance，不要求兩市場 reportDate 同日，但指定代號必須跨來源唯一。凍結同一 evaluatedAt 後，先由公司所屬市場 authoritative completed-session resolver 解析 expectedAsOf，再查 exact single-stock monthly OHLC，且只接受 selectedBarDate、bar date 與 expectedAsOf 完全相等、identity 一致、已成交且 close 為正數的 raw bar。全市場 latest OHLC 不參與這條 routing，也不 fallback 前一日。同一台北日期必須到 13:33（含）後才比較，以保守涵蓋暫緩收盤。-00:00 是未知 offset，不能當成明確時區。輸出分開 CALLER_SUPPLIED、OFFICIAL_MASTER_RAW、OFFICIAL_MARKET_RAW 與 MOPSFIN_CALC；兩份 master freshness 依各自 reportDate 揭露，resolver evidence 由 freshnessDetails 揭露，exact source 保留 dataMonth 文件身分與 selectedBarDate，dependencyLedger／workBudget 分開列 resolver logical loads、exact OHLC 及最多一次 bounded cache refresh。這不是外部 quote provider、官方／real-time 行情、fair value、進場區、目標價、評級或投資建議。

公司行動調整價格序列：get_stock_price_series 接受單一四碼 company_code、含首尾 start_date／end_date、明確 price_basis 與 include_event_ledger，最多 36 個日曆月份且不提供 public cursor。raw_unadjusted 會在單次 orchestration 內收齊最多 3 個既有 get_stock_ohlc cursor pages，但完全不查公司行動；price_index_compatible_corporate_action_adjusted 另以 TWSE／TPEx official actual-result 公司行動建立 backward series，anchorDate 固定為最後一根實際 raw bar。現金股利價格效果保留且 cash-only factor=1，所以這不是 adjusted close、股息再投資、total return 或 TSR；成交量永遠維持 raw shares。include_event_ledger 只控制 adjusted basis 是否輸出逐事件 factor、priorCloseCheck 與 markerReconciliation，raw basis 不會因此增加公司行動查詢。官方 history coverage、factor、prior close、同日多事件、identity、轉板或 changeMarker reconciliation 任一不足時，受影響 cumulativeFactor 與 adjusted OHLC 固定為 null，不可回退 raw。必須一併檢查 adjustment、coverage、sources、workBudget、fingerprint 與 meta.quality；明確歷史 range 的 freshness 為 not_applicable，不代表目前即時價格。
...
估值模型輸入：get_valuation_model_inputs 只接受單一四碼 company_code，使用目前官方 master、Mopsfin 三大報表與 authoritative completed-close dependency，整理 14 個一般企業估值資料欄位。外層 market=all current master 先唯一核對目標公司的 code、short name、market 與 exchange；同一個凍結的 evaluatedAt 再交給公司所屬市場 resolver 解析 expectedAsOf，最後只從官方 exact single-stock monthly OHLC 選取日期完全相等、identity 一致的 bar。日估值或全市場行情的 latest 日期不參與 completed-close routing；resolver 未解析、source dataMonth／selectedBarDate／bar date 不一致、exact bar 缺少、無成交、close 非正數或 identity 不符時維持 data_gap/null，不 fallback。TTM 只採 Q4 FY 或 current YTD + prior FY - prior-year YTD；每份報表必須精確核對 company identity、statement kind、共同期別、consolidation scope、唯一 semantic row role 與明示 HTML／catalog 金額單位。任何缺 statement、單位、role、market close 或 issued shares 的情況一律 status=data_gap、value=null，並保留 search-attempt lineage，不以 0、其他 row 或其他來源代填。evidenceClass 分開標示 MOPSFIN_RAW／MOPSFIN_CALC、OFFICIAL_MASTER_RAW／OFFICIAL_MARKET_RAW、OFFICIAL_CALC、MIXED_OFFICIAL_CALC 與 UNAVAILABLE；market cap 是目前 issued shares × official close，enterprise value 才混合 Mopsfin net debt。CapEx 第一版只含 PPE acquisition，normalizedFcff 是 source/sign-normalized historical FCFF proxy，不是分析師正規化或 forecast。歷史報表是查詢當下可見且可能重編的版本，不是 point-in-time filing vintage；issued shares 不是 weighted-average 或 fully diluted shares。meta.asOf 保留 master date、statement quarter 與 selectedBarDate 的 mixed cutoffs，source.dataMonth 只表示官方月資料文件身分；workBudget.authoritativeCompletedCloseCalls 分開揭露 resolver logical loads、exact OHLC attempts 與 bounded cache refresh。Mopsfin latest 財報因無可靠 expected quarter 維持 unknown；official close 只有 resolver 與 exact bar 證據一致時才可為 within_expected_window。金融公司固定 not_applicable，應使用 residual-income、dividend 或 excess-return 模型。本工具不執行 DCF、不提供隱藏 WACC／terminal growth、評級、目標價或投資建議。

市場隱含 Reverse DCF：run_reverse_dcf 只適用目前上市櫃非金融公司，沿用 get_valuation_model_inputs 的 normalized facts、完整來源 lineage，以及 resolver expectedAsOf 與 exact single-stock OHLC 共同鎖定的 completed close。模型固定 FCFF／WACC、year-end discounting 與 perpetuity-growth terminal value，一次只反解 revenue_cagr、fcff_cagr 或 terminal_operating_margin；forecast years、WACC、terminal growth、solve range、forward assumptions、sensitivity grid 與額外 EV bridge values 全部由 caller 明示，0 也不是隱藏預設。WACC 必須大於 terminal growth；缺 normalized fact／lineage、資料基礎不可行、solve range 未 bracket 或 residual 無法在 tolerance 內收斂時 fail closed。non_operating_assets 必須排除 cash，other_debt_like_items 必須排除 aggregate debt 已包含的 debt／lease roles，避免 double count。每個 sensitivity cell 重新反解並隔離無可行解；輸出分開 MOPSFIN／official facts、CALLER_ASSUMPTION 與 MODEL_OUTPUT，保留 forecast、terminal、bridge、PV、checks、source cutoffs 與完整時間順序。它不是 intrinsic value、分析師共識、目標價、評級或投資建議。
...
更新與責任：Mopsfin 每日更新一次，可能較公開資訊觀測站最新申報落後約一日；公司母體應以 list_companies 的各來源 reportDate、一般全市場價格與估值應以 dataDate、authoritative completed close 應以 resolver expectedAsOf／selectedBarDate（dataMonth 只代表月文件）、月營收應以 dataMonth／sourceReportDate，並以 meta.asOf.sourceCutoffs、coverage、sources 與 warnings 為準。本服務不是臺灣證券交易所或證券櫃檯買賣中心的官方 MCP，也不構成投資建議；重要判斷應回查官方公司名錄、行情與公開資訊觀測站原始申報。

Freshness 與時間血緣：latest 只表示查詢意圖，不會直接宣稱資料在預期窗口內。meta.asOf 分開提供 data cutoff、真正 upstream retrievedAt、MCP servedAt 與 caller-specific cache provenance；cache hit 不會改寫 retrievedAt，官方未明示 publishedAt 時維持 null。meta.quality.freshnessDetails 以中央 source-specific policy 列出 observedAsOf、expectedAsOf、lag 與 reason；官方 completed-session policy 使用 market-specific TWSE／TPEx 年度開休市日曆、當月 exact benchmark marker 與 13:33 Asia/Taipei guard，並揭露 resolver evidence 與 logical-load budget。宣稱 latest completed close 的單股工具會重用同一份 resolver evidence 作 routing 與 freshness，接著強制 exact single-stock OHLC 的 selectedBarDate 等於 expectedAsOf；source.dataMonth 只代表月資料文件，不能冒充 bar date。resolver 不可用、契約漂移、marker 未確認、跨市場日期不一致，或其他 policy 沒有可靠 expected as-of 時固定 unknown 並帶 FRESHNESS_UNVERIFIED；其他 latest policy 落後 expected as-of 時為 stale 並帶 DATA_STALE。completed-close routing 本身若無法精確綁定日期、identity 或有效正數 close 則 fail closed，不退回前一日。歷史 exact/range selector 的 latest freshness 為 not_applicable，且不呼叫 completed-session resolver。
...
export const MOPSFIN_OFFICIAL_GUIDANCE = {
  ...
  valueBasis: [
    ...
    },
    {
      dataType: "公司行動調整價格序列",
      basis:
        "get_stock_price_series 同時保留官方 raw_unadjusted OHLC，並可用 official actual-result 公司行動建立 backward price_index_compatible_corporate_action_adjusted OHLC；現金股利 factor=1、成交量為 raw shares，非 adjusted close 或 total return。必要證據不足時 adjusted 為 null，不回退 raw。",
    },
    {
      dataType: "Caller 觀察價相對官方完成收盤",
      basis:
        "analyze_observed_price 將 caller-supplied 價格與時間保留為未獨立驗證的 CALLER_SUPPLIED evidence；listed／otc master 可保留各自 reportDate 與 freshness，但指定公司 identity 必須跨來源唯一。官方基準再由 company-market resolver 固定 expectedAsOf，從 exact single-stock monthly OHLC 取同日 selected bar，不使用全市場 latest 或前一日 fallback；這只是機械價差，不是官方即時行情、fair value 或投資建議。",
    },
    {
      dataType: "市場隱含 Reverse DCF",
      basis:
        "run_reverse_dcf 以 resolver expectedAsOf 與 exact single-stock OHLC 共同鎖定的官方最近完成交易日收盤及 normalized FCFF facts，在 caller 明示全部 forward／EV-bridge assumptions 下，一次反解一個市場隱含變數；不是 intrinsic value、共識、目標價或投資建議。",
    },
    {
  ...
  interpretationNotes: [
    ...
    "get_stock_price_series 的 raw 與 adjusted 欄位必須分開解讀；adjusted 是 backward price-index-compatible 公司行動調整，不是 adjusted close 或 total return，且任何證據缺口都以 null fail closed，不得回退 raw。",
    "analyze_observed_price 的 observedPriceTwd／observedAt 來自 caller，必須與 resolver expectedAsOf、exact OHLC selectedBarDate、official completed close 及 calculated difference 分開引用；dataMonth 只是月文件身分，不得把結果描述為官方即時行情、合理價或投資建議。",
    "月營收趨勢、個股相對 benchmark 報酬與量能訊號都是透明可重算的研究代理，不是錯價證明或投資建議。",
  ],
} as const;
