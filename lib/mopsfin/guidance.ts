...
這是一個公開、唯讀、無資料庫的台灣公司財務資料 MCP Server，所有正式資料都在查詢時直接取自「公開資訊觀測站－財務比較 E 點通（Mopsfin）」。

使用順序：不知道公司代號時先呼叫 find_companies；不知道 metric_code、industry_codes、institution_codes 或可用期別時先呼叫 list_catalog。list_catalog 也會回傳官方資料範圍、各指標公式、值的計算基礎與適用限制，回答前應讀取相關 guidance。
