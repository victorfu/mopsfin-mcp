...
Caller 觀察價：analyze_observed_price 只在 caller 已提供單一 company_code、正的 observed_price_twd、明確 offset ISO observed_at 與 source_label 時使用。它以 market=all 目前公司 master 核對指定公司的 code、short name、market 與 exchange，再凍結同一 evaluatedAt：先由公司所屬市場 authoritative completed-session resolver 解析 expectedAsOf，再查 exact single-stock monthly OHLC，且只接受 selectedBarDate、bar date 與 expectedAsOf 完全相等、identity 一致、已成交且 close 為正數的 raw bar。全市場 latest OHLC 不參與這條 routing，也不 fallback 前一日。同一台北日期必須到 13:33（含）後才比較，以保守涵蓋暫緩收盤。-00:00 是未知 offset，不能當成明確時區。輸出分開 CALLER_SUPPLIED、OFFICIAL_MASTER_RAW、OFFICIAL_MARKET_RAW 與 MOPSFIN_CALC；resolver evidence 由 freshnessDetails 揭露，exact source 保留 dataMonth 文件身分與 selectedBarDate，dependencyLedger／workBudget 分開列 resolver logical loads、exact OHLC 及最多一次 bounded cache refresh。這不是外部 quote provider、官方／real-time 行情、fair value、進場區、目標價、評級或投資建議。
...
export const MOPSFIN_OFFICIAL_GUIDANCE = {
  ...
  valueBasis: [
    ...
    },
    {
      ...
      basis:
        "analyze_observed_price 將 caller-supplied 價格與時間保留為未獨立驗證的 CALLER_SUPPLIED evidence；官方基準先由 company-market resolver 固定 expectedAsOf，再從 exact single-stock monthly OHLC 取同日 selected bar，不使用全市場 latest 或前一日 fallback；這只是機械價差，不是官方即時行情、fair value 或投資建議。",
    },
    {
