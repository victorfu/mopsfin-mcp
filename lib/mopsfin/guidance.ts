...
候選快照補強：需要在同一次呼叫中完成四柱篩選及當期官方快照查核時，使用 screen_taiwan_stock_candidates_with_catalyst_snapshots。它先完整執行原本 screen，再對 screen.candidates 中所有實際 candidates 查 snapshot，數量仍受 candidate_limit 限制且最多 5 家；不論 bucket 是 research_candidate、watchlist、insufficient_data 或 deprioritized 都會查。只有 notDeepScored、notReactionScored、excluded，以及進入 deepSelected 但未形成 candidate 的公司會排除。回傳的 current snapshots 不是歷史事件，也不是分析師 consensus／consensus revision；affectsScreenScore=false，明示它不是第五柱、加分項、目標價或投資建議。原本的 screen_taiwan_stock_candidates、get_company_catalyst_snapshots 與 get_company_catalyst_events standalone tools 全部保留，可依研究流程分開呼叫。
...
export const MOPSFIN_OFFICIAL_GUIDANCE = {
  ...
  interpretationNotes: [
    ...
    "screen_taiwan_stock_candidates_with_catalyst_snapshots 只對實際最多 5 名 candidates 補查 current snapshots；affectsScreenScore=false，並非第五柱、加分項、歷史事件、分析師 consensus 或投資建議。",
